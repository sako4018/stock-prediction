  ...
  return (
    <aside className="w-56 h-screen bg-surface-1 border-r border-frame flex flex-col shrink-0">
      ...
      <div className="h-12 border-b border-frame flex items-center px-4 gap-2.5 shrink-0">
        ...
        <div>
          <span className="text-sm font-semibold text-label tracking-tight">StockPredict</span>
        </div>
      </div>
          ...
          <button
            ...
            className={`w-full flex items-center gap-2.5 px-3 py-2 rounded text-sm transition-colors ${
              activeView === item.id
                ...
                : 'text-label-dim hover:text-label hover:bg-surface-3'
            }`}
          >
      ...
      {/* Watchlist */}
      <div className="flex-1 overflow-y-auto p-3 border-t border-frame">
        <Watchlist onSelect={onSelect} currentTicker={currentTicker} />
      </div>

      {/* Footer */}
      <div className="p-3 border-t border-frame">
        <div className="flex items-center gap-1.5 text-xxs text-label-muted">
