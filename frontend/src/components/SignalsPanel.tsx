...
export default function SignalsPanel({ ticker }: SignalsPanelProps) {
  ...
  return (
    <div className="bg-alt border border-line rounded-lg p-4">
      ...
      {loading ? (
        <div className="text-txt-muted text-xs text-center py-8">Loading...</div>
      ) : signals ? (
        <div className="space-y-3">
          ...
          {[
            ...
            <div key={i} className="flex items-center justify-between py-1.5 border-b border-line last:border-0">
              <span className="text-xs text-txt-muted">{ind.name}</span>
              <div className="text-right">
                <span className="text-xs text-txt-dim tabular-nums mr-2">{ind.value}</span>
        ...
        <div className="text-txt-muted text-xs text-center py-8">No data</div>
      )}
    </div>
  )
}
